...
func checkResponse(res *http.Response, successBody interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("[service_b] API error, could not read the response body %v", err)
	}

	if res.StatusCode < 200 || res.StatusCode >= 300 {
		return fmt.Errorf("[service_b] API error, message: %v: status code %d", string(body), res.StatusCode)
	} else { // success case
		err = json.Unmarshal(body, &successBody)
		if err != nil {
			return fmt.Errorf("[service_b] API error, success case message is not decode into %T: err: %v", successBody, err)
		}
